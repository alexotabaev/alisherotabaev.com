 ...
 *
 * Про <h1>: в вёрстке Tilda заголовок — <div class='tn-atom'>, и он продублирован
 * под разрешения (t-screenmin-640px и t-screenmax-480px). Меняем тег у обоих:
 * стили и скрипты Tilda завязаны только на класс .tn-atom, тег не используется
 * нигде (сама Tilda рядом отдаёт <a class='tn-atom'>). В любой момент виден
 * ровно один из двух — значит, ровно один h1 на экране.
 */
...
function patchCase(c) {
  ...
  // Заголовок с результатом — в <h1>. Меняем оба варианта под разрешения:
  // виден всегда ровно один, значит и на экране ровно один h1.
