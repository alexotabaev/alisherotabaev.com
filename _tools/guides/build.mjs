#!/usr/bin/env node
/**
 * Генератор страницы-хаба /guides/ — каталог гайдов и разборов.
 *
 *   node _tools/guides/build.mjs
 *
 * Источник — _tools/guides/data.json. Сами гайды остаются на своих
 * адресах, хаб на них ссылается.
 *
 ...
 *
 * Падает, если гайда нет по указанному адресу: каталог, ведущий в никуда,
 * хуже отсутствующего.
 */
...
const { section, groups, guides } = JSON.parse(
...
for (const t of guides) {
  if (!known.has(t.group)) {
    throw new Error(`data.json: у гайда ${t.slug} группа «${t.group}», которой нет`);
  }
  if (!fileOf(t.url)) {
    throw new Error(`data.json: гайда ${t.slug} нет по адресу ${t.url}`);
  }
}
...
const EXTRA_CSS = `
  ...
  .gd{border:1px solid var(--line);border-radius:.7rem;background:var(--paper);padding:1.1rem 1.2rem}
  .gd h3{margin:0 0 .4rem;font-size:1.02rem;line-height:1.3}
  .gd p{margin:0 0 .8rem;color:var(--muted);font-size:.94rem}
  .gd a{color:var(--gold-dk);font-weight:600;text-decoration:none}
  .gd a:hover,.gd a:focus{text-decoration:underline}
`.trim();
...
function build() {
  const title = `Гайды и разборы — ${n} ${plural(n, 'материал', 'материала', 'материалов')}`;
  const description =
    `${n} разборов из практики: первые деньги на услугах, микропродукты, ` +
    `запуски и фокус-группы, подборки инструментов. К каждому — что вы ` +
    `отсюда унесёте.`;
  ...
  const jsonld = {
    ...
    '@graph': [
      ...
      ]),
      {
        ...
        name: section.title,
        numberOfItems: n,
        itemListElement: guides.map((t, i) => ({
          '@type': 'ListItem',
          position: i + 1,
          url: abs(t.url),
          name: t.name,
...
<article class="wrap">
  ...
  <nav aria-label="Разделы каталога">
    <ul class="groups">
${groups
  .filter((g) => guides.some((t) => t.group === g.id))
...
fs.writeFileSync(path.join(ROOT, 'guides', 'index.html'), build());

console.log(`guides/index.html: ${n} гайдов в ${groups.length} группах`);
