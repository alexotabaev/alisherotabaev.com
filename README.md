...
## `/razbor/` — длинные разборы запусков

Текст пишется в Markdown: `_tools/razbor/<slug>.md`, список разборов — `_tools/razbor/data.json`.
HTML в `razbor/` руками не правится — он перезаписывается сборкой:

```bash
node _tools/razbor/build.mjs
```

Скрины лежат в `images/razbor/<slug>/` (webp). В тексте:

- `![что на картинке](/images/razbor/<slug>/file.webp "подпись")` — скрин, несколько строк подряд — галерея;
- `📸 [СКРИН 7: что снять]` — место под скрин, которого ещё нет;
- `[ПРОВЕРИТЬ: …]`, `[ДОПИСАТЬ]` — пометки автора;
- `::: lane` … `:::` — лента этапов (внутри `#### этап`, текст и скрины).
- `::: dots` … `:::` — сетка масштаба «одна точка — N человек»: строки `всего:`, `точка:`, `подсветить:` и `подпись:`.
- `::: offer` … `:::` — врезка с призывом: строки `заголовок:`, `текст:` (можно несколько), `кнопка:`, `ссылка:`.

Пока у разбора `draft: true`, страница закрыта `noindex`, не попадает в `sitemap.xml`,
а пометки и места под скрины видны прямо на странице. С `draft: false` сборка **падает**,
если в тексте осталась хоть одна пометка или пустое место под скрин.
Всё, что ниже строки `<!-- author-notes -->` в `.md`, на страницу не попадает.

После публикации добавьте карточку разбора в `_tools/guides/data.json` (группа `launch`).
